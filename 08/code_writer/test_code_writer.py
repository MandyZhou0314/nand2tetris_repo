...
def test_write_label():
    cd = CodeWriter()
    assert cd.write_label("label", "LOOP") == ["(LOOP)"]
    assert cd.write_label("label", "CONTINUE") == ["(CONTINUE)"]
    
def test_write_goto():
    cd = CodeWriter()
    assert cd.write_goto("goto", "LOOP") == ["@LOOP", "0;JMP"]
    assert cd.write_goto("goto", "CONTINUE") == ["@CONTINUE", "0;JMP"]

def test_write_if():
    cd = CodeWriter()
    assert cd.write_if("if-goto", "LOOP") == []